...
def get_acme_client(key: jose.JWK) -> acme.client.ClientV2:
    """Get an ACME client"""
...
def create_secret(service_client, arn, token):
    ...
    try:
        ...
        log(log_info, "TODO: actial code", False)
    
        # get the ACME client
        new_rsa_key = hazmat.primitives.asymmetric.rsa.generate_private_key(
            public_exponent=65537,
            key_size=2048,
            backend=hazmat.backends.default_backend(),
        )
        new_key = jose.JWKRSA(key=jose.ComparableRSAKey(new_rsa_key))
        acme_client = get_acme_client(new_key)
        if True:  # Handle updating key later
            msg = acme.messages.NewRegistration.from_data(
                email=os.environ['ACME_EMAIL'],
                terms_of_service_agreed=True,
            )
            registration = acme_client.new_account(msg)
